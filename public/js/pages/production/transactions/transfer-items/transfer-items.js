...
$(function () {
  ...
  $('#div_code').on('change', function () {
    //checkIfSameDivCode();
    $('#div_code_id').val($(this).find("option:selected").attr('data-div_code_id'));
    var p_process = $("#div_code").find("option:selected").text();

    if (p_process.charAt(0) == 'X') {
      $('#new_code').prop('readonly', false);
    } else {
      $('#new_code').prop('readonly', true);
    }
  });
  ...
  $("#frm_transfer_items").on('submit', function (e) {
    e.preventDefault();
    transfer_item_arr = [];
    var totalqty = parseInt($('#UnprocessTransfer').val()) + parseInt($('#qty').val()); // if ($('#process').val() == $("#curr_process").find("option:selected").text() && $('#userDivCode').val() == $("#div_code").find("option:selected").text()) {
    ...
    if (parseFloat($('#qty').val()) > parseFloat($('#unprocessed').val())) {
      msg("You cannot transfer more quantity than the available quantity.", "warning");
    } else if ($('#qty').val() < 0) {
      msg("Please Input valid number", "warning");
    } else {
      var p_div_code = $("#div_code").find("option:selected").text();

      if (p_div_code.charAt(0) == 'W') {
        msg("Saved to Warehouse", "warning");
      }
  ...
  $('#btn_delete_set').on('click', function () {
    var chkArray = [];
    var table = $('#tbl_transfer_entry').DataTable();
  ...
  swal({
    ...
    if (isConfirm) {
      ...
      $.ajax({
        ...
        $('.loadingOverlay').hide();
        var moved = data.moved_data;

        if (moved.length > 0) {
          $('.loadingOverlay-modal').show();
          movedItemsDatatable(moved);
        }
      }).fail(function (xhr, textStatus, errorThrown) {
...
function movedItemsDatatable(arrayData) {
  var tbl_transfer_entry = $('#tbl_moved_data').DataTable();
  tbl_transfer_entry.clear();
  tbl_transfer_entry.destroy();
  tbl_transfer_entry = $('#tbl_moved_data').DataTable({
    data: arrayData,
    processing: true,
    searching: true,
    // paging: false,
    ...
    fnInitComplete: function fnInitComplete() {
      $('.loadingOverlay-modal').hide();
      $('.dataTables_scrollBody').slimscroll();
      $("#tbl_moved_data").wrap("<div style='overflow:auto; width:100%;position:relative;'></div>");
      $('#modal_moved_data').modal('show');
    }
  });
}
...
function SaveProcess() {
  ...
  if (isValid) {
    $.ajax({
      ...
      msg(data.msg, data.status); //if(data.status == ""){
...
module.exports = __webpack_require__(/*! C:\xampp\htdocs\en-pms\resources\assets\js\pages\production\transactions\transfer-items\transfer-items.js */"./resources/assets/js/pages/production/transactions/transfer-items/transfer-items.js");
