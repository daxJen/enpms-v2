...
$(function () {
  ...
  $('#btn_search_excel').on('click', function () {
    var DownloadURL = excelSearchFilterURL + "?srch_date_upload_from=" + $('#srch_date_upload_from').val() + "&srch_date_upload_to=" + $('#srch_date_upload_to').val() + "&srch_sc_no=" + $('#srch_sc_no').val() + "&srch_prod_code=" + $('#srch_prod_code').val() + "&srch_description=" + $('#srch_description').val() + "&srch_po=" + $('#srch_po').val();
    var percentage = 10;
    $('#progress').show();
    ...
    req.onreadystatechange = function () {
      if (req.readyState == 2 && req.status == 200) {
        ...
        var filename = matches != null && matches[1] ? matches[1] : 'Product_Master.xlsx'; // var filename = $(that).data('filename');
...
function getUploadedProducts() {
  uploadedProductsTable(datatableUpload); // $('.loadingOverlay').show();
  // $.ajax({
  // 	url: datatableUpload,
  // 	type: 'GET',
  // 	dataType: 'JSON',
