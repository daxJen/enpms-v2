...
class PDFController extends Controller
{
	...
	public function TSReprint(Request $req)
	{
		$jo_sequence = explode(',', $req->jo_sequence);

		$prod_ids = [];
		$travel_sheets = [];

		$head_travel_sheet = DB::table('prod_travel_sheets')
			->whereIn('jo_sequence', $jo_sequence)
			->select('id', 'jo_no', 'jo_sequence', 'prod_code')
			->orderBy('id','asc')
			->get();
		$iso = $this->_helper->getISO($req->iso_control);

		foreach ($head_travel_sheet as $key => $ts) {
			array_push($travel_sheets, [
				'id' => $ts->id,
				'date' => date('M. d, Y'),
				'header' => DB::table('prod_travel_sheets')->where('id', $ts->id)->get(),
				'process' => DB::select("SELECT tp.travel_sheet_id,
												tp.process as process_name,
												pd.div_name,
												tp.div_code,
												tp.sequence,
												tp.travel_sheet_remarks as remarks
										FROM enpms.prod_travel_sheet_processes as tp
										inner join ppc_divisions as pd
										on tp.div_code = pd.div_code
										where tp.travel_sheet_id = ". $ts->id."
										group by tp.travel_sheet_id,
												tp.process,
												pd.div_name,
												tp.div_code,
												tp.sequence
		...
		$pdf = PDF::loadView('pdf.travel_sheet_reprint', ['data' => $travel_sheets]);
		return $pdf->setPaper('A4')
			->setOption('margin-top', 6)
			->setOption('margin-left', 5)
			->setOption('margin-right', 5)
