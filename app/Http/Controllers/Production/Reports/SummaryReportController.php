    ...
    public function downloadExcel(Request $req)
    {
        ...
        Excel::create('PRODUCTION_SUMMARY_REPORT_'.$date, function($excel)use($data)
        {
            $excel->sheet('Production Summary Report', function($sheet) use($data)
            {
                ...
                $sheet->cells('A4:AA5', function($cells) {
                    $cells->setFont([
                        'family'     => 'Calibri',
                        'size'       => '11',
                        'bold'       =>  true,
                ...
                $sheet->cell('Y5', function($cell) {
                    $cell->setAlignment('center');
                    $cell->setValue("REWORK");
                    $cell->setBorder('thick','thick','thick','thick');
                });
                $sheet->cell('Z5', function($cell) {
                    $cell->setAlignment('center');
                    $cell->setValue("SCRAP");
                    $cell->setBorder('thick','thick','thick','thick');
                });
                ...
                foreach ($data as $key => $dt) {
                    ...
                    $sheet->cell('W'.$row, function($cell) use($dt) {
                        $cell->setValue($dt->walloy_mix);
                        $cell->setBorder('thin','thin','thin','thin');
                    });
                    $sheet->cell('X'.$row, function($cell) use($dt) {
                        $cell->setValue($dt->wconver);
                        $cell->setBorder('thin','thin','thin','thin');
                    });

                    //rejection rate
                    $sheet->cell('Y' . $row, function ($cell) use ($dt) {
                        $cell->setValue($dt->rrework);
                        $cell->setBorder('thin', 'thin', 'thin', 'thin');
                    });
                    $sheet->cell('Z' . $row, function ($cell) use ($dt) {
                        $cell->setValue($dt->rscrap);
                        $cell->setBorder('thin', 'thin', 'thin', 'thin');
                    });
                    $sheet->cell('AA' . $row, function ($cell) use ($dt) {
