...
class MaterialMasterController extends Controller
{
	...
	{
		$mat = DB::select("SELECT pmc.id as id, pmc.material_type as material_type, 
						  pmc.material_code as material_code, pmc.code_description as code_description,
       					  u.nickname as create_user, pmc.item as item, pmc.alloy as alloy, 
       					  pmc.schedule as schedule, pmc.size as size, pmc.std_weight as std_weight, 
       					  pmc.updated_at as updated_at, pmc.disabled as disabled
					FROM enpms.ppc_material_codes as pmc
					LEFT JOIN enpms.users as u ON u.id = pmc.create_user");
