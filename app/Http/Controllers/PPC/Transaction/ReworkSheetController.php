...
use App\PpcReworkSheet;
use App\PpcReworkSheetProducts;
use App\PpcReworkSheetProcess;
...
class ReworkSheetController extends Controller
{
    ...
    public function getJoDetails()
    {
        ...
        // $info = PpcReworkInfo::select('id','trans_no','status','created_at')
        //                                     ->where("id", "=", function ($query) {
        //                                         $query->select(DB::raw(" MIN(id)"))
        //                                           ->from('prod_travel_sheets')
        ...
        //$sheet_id = (int)$req->sheet_id;
            // $details = DB::select(
            //                             DB::raw("CALL ppc_getrework(".Auth::user()->id.(int)$req->sheet_id.")")
            //                         );

            
            // return DataTables::of($data)
            //             ->addColumn('action', function($data) {
            //                 return "<button type='button' class='btn btn-sm bg-green open_rework_sheet_modal' 
            //                             data-jo_no='".$data->jo_no."' data-prod_code='".$data->product_code."' 
            //                             data-description='".$data->description."' data-based_qty='".$data->based_qty."' 
            //                             data-rework_qty='".$data->rework_qty."'  data-process='".$data->process."' 
            //                             data-div_code='".$data->div_code."'  data-create_user='".$data->create_user."' 
                                        
            //                             title='Prepare Rework Sheet'><i class='fa fa-file-text-o'></i> </button>";
            //             })
            //             ->make(true);
     
            $data = DB::select("SELECT ts.id as id, ts.jo_no as jo_no, ts.prod_code as prod_code, ts.description as description, 
                    (tsp.unprocessed + tsp.good + tsp.rework + tsp.scrap + tsp.convert) as based_qty,
                    tsp.rework as rework_qty, tsp.process as fr_process, tsp.div_code as div_code, ts.create_user as create_user
                    FROM enpms.prod_travel_sheets ts
                    LEFT JOIN enpms.prod_travel_sheet_processes tsp ON tsp.travel_sheet_id = ts.id
                    LEFT JOIN enpms.users u ON u.id = ts.create_user
                    WHERE rework <> 0");
    ...
    public function getReworkSheetData(Request $req)
    {
        $data = [
            'prod' => '',
            'process' => '',
        ...
        $rew_sheet = PpcReworkSheetProducts::where('rework_sheet_id',$req->id)
                                        ->select('prod_code')
                                        ->first();

        $process = PpcReworkSheetProcess::where('rework_sheet_id',$req->id)->get();
        $process_set = PpcReworkSheetProcess::where('rework_sheet_id',$req->id)
                            ->select('set')->first();

        $data = [
            'prod' => $prod,
            'process' => $process,
            'prod_code' => (isset($rew_sheet->prod_code))? $rew_sheet->prod_code: '',
